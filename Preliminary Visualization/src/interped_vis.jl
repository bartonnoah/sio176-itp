...
for (name, data, unit, color) in zip(varnames, vardata, varunits, colors)
    p = contourf(times_expanded, depth, data; title = "Interped $name vs depth and time", xlabel = "time", ylabel = "Depth (m)", colorbar_title = "$name ($unit)", yflip=true, c = color, dpi = 1200)
    display(p)
